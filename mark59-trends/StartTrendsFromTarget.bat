...
IF [%DATABASE%] == [] (
	...
	java -jar ./target/mark59-trends.war --spring.profiles.active=h2 --port=8083 
)
...
IF "%DATABASE%" == "H2TCPSERVER" (
	...
	java -jar ./target/mark59-trends.war --spring.profiles.active=h2tcpserver --port=8083 --h2.port=9092 
)

IF "%DATABASE%" == "H2MEM" (
	rem Using H2 in memory  Starting Trend Analysis. default application server port 
	java -jar ./target/mark59-trends.war --spring.profiles.active=h2mem --port=8083 
)
